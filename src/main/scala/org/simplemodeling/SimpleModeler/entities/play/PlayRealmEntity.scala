 ...
 */
class PlayRealmEntity(aIn: GDataSource, aOut: GDataSource, aContext: GEntityContext) extends PRealmEntity(aIn, aOut, aContext) {
...
class PlayRealmEntityClass extends GEntityClass {
  type Instance_TYPE = PlayRealmEntity

  override def accept_Suffix(suffix: String): Boolean = suffix == "play.d"

  override def reconstitute_DataSource(aDataSource: GDataSource, aContext: GEntityContext): Option[Instance_TYPE] = Some(new PlayRealmEntity(aDataSource, aContext))
}
