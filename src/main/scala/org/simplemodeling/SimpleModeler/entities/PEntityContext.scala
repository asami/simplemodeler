...
//    require (qname != "app", "qname should not be \"app\" which means just default package name without class name.")
  ...
  def sqlMaker(entity: PEntityEntity, isTarget: PAttribute => Boolean): SqlMaker = {
    new EntitySqlMaker(entity, isTarget)(this)
  }

  def sqlMaker(doc: PDocumentEntity, isTarget: PAttribute => Boolean): SqlMaker = {
    new DocumentSqlMaker(doc, isTarget)(this)
  }
  ...
  def sqlJoinBackReferenceColumnName(base: PObjectEntity, backref: PObjectEntity): String = {
    backref.wholeAttributesWithoutId.find(x => {
      x.getEntity.map(base.isAncestor) | false
