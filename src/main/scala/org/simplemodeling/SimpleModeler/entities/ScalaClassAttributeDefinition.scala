...
class ScalaClassAttributeDefinition(
  ...
  smaker: ScalaMaker) extends GenericClassAttributeDefinition(pContext, aspects, attr, owner) with ScalaMakerHolder {
...
//          sm_assign("doc.%s_now".format(varName), "false")
        ...
        case p: PEntityPartType => {
          sm_pln("doc.%s = this.%s.make_document();", varName, refVarName)
        }
        ...
        case p: PEntityPartType => {
          ...
          sm_if_not_null("this." + partVarName) {
            sm_for(java_element_type() + " element: " + partVarName) {
              sm_pln("doc.%s.add(element.make_document());", attrName)
            }
          }
        }
        case v: PByteType    => update("%s.byteValue()")
...
/*
          ...
          def deep {
            ...
            sm_if_not_null(refVarName) {
              sm_assign("doc.%s".format(varName), "%s.make_document()", refVarName)
            }
          }
          ...
          def shallow {
            sm_pln("fill_%s();".format(attrName))
            sm_if_not_null(refVarName) {
              sm_assign("doc.%s".format(varName), "%s.make_document_shallow()", refVarName)
            }
          }
          ...
          if (is_owned_property()) {
            ...
            sm_if_not_null(refVarName) {
              get_id
              sm_pln("doc.%s = %s.make_document();".format(varName, refVarName))
            }
 ...
 */
  def method_document_make_single_entity_simple(e: PEntityType) {
    sm_pln("doc.%s = this.%s.make_document();", varName, varName)
  }
...
/*
          ...
          def deep {
            ...
            sm_for("%s entity: %s".format(e.entity.name, refVarName)) {
              sm_pln("doc.%s.add(entity.make_document());".format(varName))
            }
          }
          ...
          def shallow {
            sm_pln("fill_%s();".format(attrName))
            sm_for("%s entity: %s".format(e.entity.name, refVarName)) {
              sm_pln("doc.%s.add(entity.make_document_shallow());".format(varName))
            }
          }
