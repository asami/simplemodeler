 ...
 */
class GwtTimeInputFieldEntity(val gwtContext: GwtEntityContext) extends GEntity(gwtContext) {
...
public class TimeInputField extends HorizontalPanel implements IInputField {
    ...
    public Date getValue() {
        ...
        if (time != null) {
            ...
            return Util.string2time(timeText.getText());
        }
    }
    ...
    public void setValue(Object value) {
        if (value == null) {
            ...
            time = Util.string2time((String) value);
        } else {
            throw new IllegalArgumentException(value + " is not Date or String");
        }
        if (time != null) {
            timeText.setValue(Util.time2text(time));
        } else {
