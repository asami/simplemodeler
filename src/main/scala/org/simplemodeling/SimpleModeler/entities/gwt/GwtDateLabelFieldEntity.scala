 ...
 */
class GwtDateLabelFieldEntity(val gwtContext: GwtEntityContext) extends GEntity(gwtContext) {
...
public class DateLabelField extends Label implements ILabelField {
    private Date date = null;

    public DateLabelField() {
    }

    public DateLabelField(Date date) {
        this.date = date;
        update();
    }

    private void update() {
        setText(formatDefault(date));
    }
