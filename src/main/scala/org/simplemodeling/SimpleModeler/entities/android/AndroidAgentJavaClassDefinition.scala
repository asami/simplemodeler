 ...
 */
class AndroidAgentJavaClassDefinition(
  pContext: PEntityContext,
  aspects: Seq[JavaAspect],
  pobject: PObjectEntity
) extends AgentJavaClassDefinition(pContext, aspects, pobject) {
  customBaseName = Some("GAgent<%s, %s, %s>".format(contextName, errorModelName, modelName))
